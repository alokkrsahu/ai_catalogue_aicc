...
\begin{abstract}
Building production-ready document-intelligent chatbots from multi-agent workflows remains challenging due to fragmentation across existing tools: multi-agent frameworks provide coordination but lack per-agent document awareness; visual workflow builders offer accessibility but couple RAG configuration to entire workflows rather than individual agents; deployment requires custom infrastructure. We present \textbf{IntelliDoc\footnote{\url{https://github.com/submissionanonymous4-byte/intellidoc}}}, an open-source framework that bridges visual workflow design to deployable chatbots with five key contributions: (1) \textbf{DocAware}, a per-agent RAG system enabling each agent to independently configure search methods and content filters for specialised document access; (2) \textbf{Dual-Mode Human-in-the-Loop}, distinguishing between administrator oversight and end-user interaction contexts; (3) \textbf{Intelligent Delegation}, an LLM-based query decomposition and capability-matching system for the Group Chat Manager that dynamically routes subqueries to specialised delegates; (4) \textbf{Workflow Evaluation Platform}, providing automated quality assessment using lexical and semantic metrics; and (5) \textbf{Workflow-to-Chatbot Deployment}, translating visual workflows into embeddable chatbots with origin management and rate limiting.
\end{abstract}

\section{Introduction}

Large language models have enabled multi-agent systems where specialised agents collaborate on complex tasks, yet translating these into production-ready, document-intelligent chatbots remains fragmented. A customer service workflow requiring a Legal Agent retrieving contracts, a Technical Agent accessing documentation, and administrator approval before customer delivery currently demands separate tools: a multi-agent framework, RAG system, custom human-in-the-loop code, and bespoke deployment infrastructure.

This fragmentation creates three gaps: (1) visual workflow tools configure RAG at the \textit{workflow level}, preventing agent specialisation where Legal and Technical agents search different folders; (2) human-in-the-loop implementations lack distinction between administrator oversight and end-user chatbot interaction; (3) translating visual designs into production chatbots requires custom pipelines most frameworks omit.
...
DocAware distinguishes IntelliDoc from workflow-level RAG approaches by enabling each agent to independently configure: (1) whether DocAware is enabled, (2) which search method to use, (3) search parameters, and (4) content filters specifying accessible folders and files (see Figure~\ref{fig:docaware-config} in Appendix~\ref{app:figures}).
...
The UserProxyAgent implements human-in-the-loop functionality with two distinct modes addressing different deployment contexts (see Figure~\ref{fig:hitl-modes} in Appendix~\ref{app:figures}).
...
We evaluate IntelliDoc's runtime performance focusing on the overhead introduced by per-agent DocAware retrieval. Experiments use a single AI Assistant agent with OpenAI GPT-3.5-turbo processing 9 question-answer pairs from a domain-specific dataset.

\subsection{Experimental Setup}

We compare execution time under two conditions: (1) \textit{Baseline} configuration without DocAware enabled, and (2) \textit{DocAware-enabled} configuration using hybrid search with content filters restricting retrieval to a single document. The workflow consists of three nodes (Start, AI Assistant, End) executing sequentially. All experiments run on the same infrastructure with Django 5.2 backend, Milvus 2.6.0 vector database, and all-MiniLM-L6-v2 embeddings (384 dimensions).

\subsection{Execution Time Analysis}

Table~\ref{tab:performance} presents execution time statistics across both configurations.
...
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Without DocAware} & \textbf{With DocAware} \\
\midrule
Average Time & 1.709s & 2.787s \\
Min Time & 1.193s & 2.601s \\
Max Time & 3.377s & 3.071s \\
Test Cases & 9 & 9 \\
\midrule
\textbf{DocAware Overhead} & & \\
Search Duration & - & $\sim$45ms \\
Documents Retrieved & 0 & 1 per query \\
Total Overhead & - & +1.078s (+63\%) \\
\bottomrule
\end{tabular}
\caption{Execution time comparison for single-agent workflow with and without DocAware retrieval. DocAware introduces 63\% overhead, though actual vector search accounts for only 45ms; the remainder stems from LLM processing expanded prompts with document context.}
\label{tab:performance}
\end{table}

The DocAware-enabled configuration exhibits a 1.078-second average overhead (63\% increase) compared to the baseline. However, the actual vector search operation contributes only approximately 45 milliseconds per query. The dominant overhead factor is LLM processing time for expanded prompts containing retrieved document context (averaging 648 characters). This demonstrates that while per-agent RAG retrieval introduces measurable latency, the vector search itself remains efficient, with the primary cost being downstream LLM inference on augmented context.
...
This work utilised AI-assisted programming tools. We acknowledge the use of Cursor \citep{cursor2024}, an AI-powered code editor, for code completion and refactoring assistance. We also utilised large language model assistants including Claude \citep{anthropic2024claude}, ChatGPT \citep{openai2023gpt4}, and Gemini \citep{team2023gemini} for code generation, debugging support, documentation drafting, and iterative development feedback. These tools contributed to accelerating the implementation process while all generated code was reviewed and validated by the authors.
...
\begin{figure}[!t]
    ...
    \caption{Dual-mode Human-in-the-Loop interface (Section~\ref{sec:hitl}). Admin-in-the-Loop or User-in-the-Loop selection interface.}
