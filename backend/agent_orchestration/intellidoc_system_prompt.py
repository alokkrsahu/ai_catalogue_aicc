...
CITATION_FORMAT_BLOCK = (
    ...
    "Append a citations block at the very END of your entire response, wrapped between the EXACT "
    "markers `---CITATIONS---` on the opening line and `---END_CITATIONS---` on the closing line. "
    "BOTH markers MUST include the three leading AND three trailing dashes. Do NOT shorten the opener "
    "to bare `CITATIONS` (without dashes) — the frontend parser looks for the full delimiter and will "
    "otherwise render the raw JSON block as visible text in the chat. "
    ...
    "The citations block must be valid JSON in this exact format (note: both opening and closing "
    "markers use three dashes on each side):\n"
    "---CITATIONS---\n"
