...
<script lang="ts">
  ...
  function generateEmbedCode(): string {
    ...
    htmlParts.push('  const conversation = [];');
    htmlParts.push('');
    ...
    htmlParts.push('    conversation.push({ role: \'user\', content: text });');
    htmlParts.push('');
    ...
    htmlParts.push('      if (data.status === \'success\') {');
    htmlParts.push('        const reply = data.response || \'(No response)\';');
    htmlParts.push('        appendMessage(\'assistant\', reply);');
    htmlParts.push('        conversation.push({ role: \'assistant\', content: reply });');
    ...
    htmlParts.push('    if (e.key === \'Enter\' && !e.shiftKey) {');
    ...
    htmlParts.push('  conversation.push({ role: \'assistant\', content: INITIAL_GREETING });');
    htmlParts.push('</' + 'script>');
    htmlParts.push('</body>');
    htmlParts.push('</html>');
